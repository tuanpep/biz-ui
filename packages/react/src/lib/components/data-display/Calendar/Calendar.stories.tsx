...
const SingleModeCalendar = (args: Story["args"]) => {
  const [date, setDate] = useState<Date | undefined>(
    args?.selected || addDays(new Date(), 2),
  );
  return (
...
const RangeModeCalendar = (args: Story["args"]) => {
  ...
  const handleSelect = (day: Date) => {
    if (!range || (range.start && range.end)) {
      ...
      const start =
        range.start && isBefore(range.start, day) ? range.start : day;
      const end =
        range.start && isBefore(range.start, day) ? day : (range.start ?? null);
      setRange({ start, end });
    }
  };

  return (
    <Calendar {...args} mode="range" range={range} onSelect={handleSelect} />
  );
};
