...
describe('FileUploader', () => {
  ...
  it('renders with description', () => {
    render(<FileUploader label="Upload" description="Select files to upload" />);
  ...
  it('handles disabled state', () => {
    ...
    const dropContainer = screen.getByRole('button', { name: /drag and drop/i });
  ...
  it('shows file size correctly', () => {
    const files: FileUploaderFile[] = [
      { uuid: '1', name: 'large.txt', size: 1024 * 1024 * 5, type: 'text/plain' }, // 5 MB
    ];
    render(<FileUploader files={files} />);
  ...
  it('shows error state on file item', () => {
    const files: FileUploaderFile[] = [
      { uuid: '1', name: 'error.txt', size: 1024, type: 'text/plain', status: 'error', errorMessage: 'Upload failed' },
    ];
    render(<FileUploader files={files} />);
    expect(screen.getByText('Upload failed')).toBeInTheDocument();
  });
  ...
  it('shows uploading state on file item', () => {
    const files: FileUploaderFile[] = [
      { uuid: '1', name: 'uploading.txt', size: 1024, type: 'text/plain', status: 'uploading', progress: 50 },
    ];
    render(<FileUploader files={files} />);
...
describe('FileUploaderButton', () => {
  ...
  it('renders different variants', () => {
    const { rerender } = render(<FileUploaderButton variant="primary">Primary</FileUploaderButton>);
    expect(screen.getByText('Primary')).toBeInTheDocument();

    rerender(<FileUploaderButton variant="secondary">Secondary</FileUploaderButton>);
  ...
  it('renders different sizes', () => {
    const { rerender } = render(<FileUploaderButton size="sm">Small</FileUploaderButton>);
...
describe('FileUploaderItem', () => {
  ...
  it('shows error state', () => {
    const errorFile = { ...defaultFile, status: 'error' as const, errorMessage: 'Failed' };
    render(<FileUploaderItem file={errorFile} />);
    expect(screen.getByText('Failed')).toBeInTheDocument();
  });

  it('shows uploading state with progress', () => {
    const uploadingFile = { ...defaultFile, status: 'uploading' as const, progress: 75 };
...
describe('FileUploaderSkeleton', () => {
  ...
  it('renders skeleton with description', () => {
    render(<FileUploaderSkeleton hasDescription data-testid="skeleton-with-desc" />);
  ...
  it('renders skeleton with file items', () => {
    render(<FileUploaderSkeleton itemCount={3} data-testid="skeleton-with-items" />);
