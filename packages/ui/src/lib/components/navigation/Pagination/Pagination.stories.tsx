...
const DefaultPagination = (args: Story['args']) => {
    ...
    return <Pagination {...args} total={args?.total || 10} page={page} onChange={setPage} />;
};
...
const FirstLastPagination = (args: Story['args']) => {
    const [page, setPage] = useState(args?.page || 1);
    return <Pagination {...args} total={args?.total || 20} page={page} onChange={setPage} showFirstLast />;
};
...
const NavOnlyPagination = (args: PaginationNavProps) => {
    const [page, setPage] = useState(args?.page || 1);
    return <PaginationNav {...args} total={args?.total || 5} page={page} onChange={setPage} />;
};
