...
describe('addPrefix', () => {
  ...
  it('should handle array of class names', () => {
    expect(addPrefix(['button', 'primary'], 'biz')).toBe('biz-button biz-primary');
  });
});
...
describe('FeatureFlagsProvider', () => {
  ...
  it('should provide custom feature flags', () => {
    ...
    const wrapper = ({ children }: { children: React.ReactNode }) => (
      <FeatureFlagsProvider flags={customFlags}>{children}</FeatureFlagsProvider>
    );
  ...
  it('should use useFeatureFlag for individual flags', () => {
    ...
    const { result } = renderHook(() => useFeatureFlag('enable-css-grid'), { wrapper });
  ...
  it('should return default value for undefined flags when wrapped', () => {
    ...
    const { result } = renderHook(() => useFeatureFlag('enable-ai-features'), { wrapper });
    expect(result.current).toBe(false);
  });
});
